...
coupling01 = make_coupling(0.1)
shape0 = kurkela_shape(PX, PY, PETA, tau0)
A10 = normalize_amplitude(shape0, PX, PY, PETA, dpx, dpy, dpeta, coupling10)
A01 = normalize_amplitude(shape0, PX, PY, PETA, dpx, dpy, dpeta, coupling01)

def evolve(coupling, A, case, tau_vals):
...
PL_B0, PT_B0 = evolve(coupling10, A10, None, tau_vals)
PL_lam10, PT_lam10 = evolve(coupling10, A10, case, tau_vals)
PL_lam01, PT_lam01 = evolve(coupling01, A01, case, tau_vals)

fig, ax = plt.subplots(figsize=(5, 5.5))
ax.semilogy(tau_fm_vals, PL_B0 / PT_B0, "C0-", linewidth=2, label="$B=0$")
ax.semilogy(tau_fm_vals, PL_lam10 / PT_lam10, "r-", linewidth=2, label=r"$B\neq0$, $\lambda=10$")
ax.semilogy(tau_fm_vals, PL_lam01 / PT_lam01, "r--", linewidth=2, label=r"$B\neq0$, $\lambda=0.1$")
ax.set_xlabel(r"$\tau$ [fm]")
ax.set_ylabel(r"$P_L/P_T$")
ax.grid(alpha=0.3, which="both")
curve_legend = ax.legend(fontsize=9, loc="lower left")
...
info_label = (r"60-80% centrality" + "\n" +
              fr"$B_{{\rm peak}}$={B_peak_GeV2:.3f} GeV$^2$"
              fr" ({B_peak_GeV2/m_pi2:.1f} $m_\pi^2$)")
ax.legend(handles=[Line2D([], [], linestyle="none")], labels=[info_label],
          fontsize=9, loc="upper right", handlelength=0, handletextpad=0)

fig.tight_layout()
fig.savefig("LHC_PLPT_lambda.png", dpi=150, bbox_inches="tight")
