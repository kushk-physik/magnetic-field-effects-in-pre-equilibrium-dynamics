...
# xi changes the IC shape itself (not just its amplitude), so each xi needs its own
# shape/amplitude/B=0 baseline -- xi=1 is the isotropic IC (equilibrium check)
A_of_xi, B0_of_xi = {}, {}
for xi_val in xi_values:
    shape0 = kurkela_shape(PX, PY, PETA, tau0, xi_val)
    A_of_xi[xi_val] = normalize_amplitude(shape0, PX, PY, PETA, dpx, dpy, dpeta)
    B0_of_xi[xi_val] = evolve(None, A_of_xi[xi_val], xi_val, tau_vals)
...
for ax, (cent_name, case) in zip(axes, RHIC.items()):
    for xi_val in xi_values:
        ...
        PL_B0, PT_B0 = B0_of_xi[xi_val]
        PL_on, PT_on = evolve(case, A_of_xi[xi_val], xi_val, tau_vals)
...
fig.tight_layout()
fig.savefig("RHIC_PLPT_xi.png", dpi=150, bbox_inches="tight")
